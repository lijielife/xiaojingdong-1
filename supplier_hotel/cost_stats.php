...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
        ...
        /* 文件标题 */
        echo ecs_iconv(EC_CHARSET, 'GB2312', $_REQUEST['start_date']. $_LANG['to'] .$_REQUEST['end_date']. $_LANG['cost_stats']) . "\t\n";

        /* 商品名称,订单号,商品数量,销售价格,销售日期 */
        echo ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['goods_name']) . "\t";
        echo ecs_iconv(EC_CHARSET, 'GB2312', '售出数量') . "\t";
        echo ecs_iconv(EC_CHARSET, 'GB2312', '成本价格') . "\t";
        ...
        foreach ($goods_sales_list['sale_list_data'] AS $key => $value)
        {
            ...
            echo ecs_iconv(EC_CHARSET, 'GB2312', $value['cb_price']) . "\t";
            echo "\n";
        }
        exit;
    }
    ...
    make_json_result($smarty->fetch('cost_stats.htm'), '', array('filter' => $sale_list_data['filter'], 'page_count' => $sale_list_data['page_count']));
}
/*------------------------------------------------------ */
//--商品明细列表
/*------------------------------------------------------ */
else
{
    ...
    if (!isset($_REQUEST['goods_id']))
    {
        ...
        $smarty->assign('goods_sales_list', '');
    }
    else
    {
        $goods_id = $_REQUEST['goods_id'];
        $sale_list_data = get_sale_list($goods_id);
        $smarty->assign('filter',       $sale_list_data['filter']);
        $smarty->assign('record_count', $sale_list_data['record_count']);
        $smarty->assign('page_count',   $sale_list_data['page_count']);
        $smarty->assign('goods_sales_list', $sale_list_data['sale_list_data']);
    ...
    */
    ...
    $smarty->assign('page_count',   $sale_list_data['page_count']);
    $smarty->assign('goods_sales_list', $sale_list_data['sale_list_data']);
    $smarty->assign('ur_here',          $_LANG['cost_stats']);
    $smarty->assign('full_page',        1);
    $smarty->assign('start_date',       local_date('Y-m-d', $start_date));
    $smarty->assign('end_date',         local_date('Y-m-d', $end_date));

    $smarty->assign('cfg_lang',     $_CFG['lang']);
    $smarty->assign('action_link',  array('text' => $_LANG['down_cost'],'href'=>'#download'));

    /* 显示页面 */
    assign_query_info();
    $smarty->display('cost_stats.htm');
}
/*------------------------------------------------------ */
//--获取销售明细需要的函数
/*------------------------------------------------------ */
...
function get_sale_list($is_pagination=true){
    ...
    /* 时间参数 */
    $filter['start_date'] = empty($_REQUEST['start_date']) ? local_strtotime('-377 days') : local_strtotime($_REQUEST['start_date']);
    $filter['end_date'] = empty($_REQUEST['end_date']) ? local_strtotime('+1 day') : local_strtotime($_REQUEST['end_date']);
  
    /* 查询数据的条件 */
    $where = " WHERE og.order_id = oi.order_id". order_query_sql('finished', 'oi.') .
             // " AND og.goods_id='goods_id'".
             " AND oi.add_time >= '".$filter['start_date']."' AND oi.add_time < '" . ($filter['end_date'] + 86400) . "' AND oi.supplier_id='$_SESSION[supplier_id]' GROUP BY og.goods_id";
    ...
    $sql = 'SELECT og.goods_id,  og.goods_name, SUM(goods_number) AS goods_num,SUM(og.goods_number * og.cb_price) AS cb_price FROM '.
            $GLOBALS['ecs']->table('order_goods')." AS og, ".$GLOBALS['ecs']->table('order_info')." AS oi ".
           $where. " ORDER BY og.goods_id DESC";
    if ($is_pagination)
    {
    ...
    foreach ($sale_list_data as $key => $item)
    {
        ...
        if($item['cb_price'] == 0)
        {
            $sale_list_data[$key]['cb_price'] = '未录入成本价';
        }
    }
