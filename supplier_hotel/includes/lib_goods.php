        ...
        /* 记录总数 */
            ...
            if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
                ...
                $sql = "SELECT COUNT(distinct(g.goods_id)) FROM " .$GLOBALS['ecs']->table('goods'). " AS g,"
                        ...
                        $GLOBALS['ecs']->table('virtual_goods_district')." as dig".
                        " WHERE dis.supplier_id = ".$_SESSION['supplier_id']."   and dis.goods_id = g.goods_id and dis.district_id = dig.district_id AND is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where";
                ...
                $sql = "SELECT COUNT(distinct(goods_id)) FROM " .$GLOBALS['ecs']->table('goods'). "  WHERE is_delete='$is_delete' AND supplier_id='". $_SESSION['supplier_id'] ."' $where";
    ...
    /* 代码增加_虚拟团购_START  www.68ecshop.com */
         if(!$real_goods && $filter['extension_code'] == 'virtual_good'){
             $sql = "SELECT distinct(g.goods_id), goods_name, goods_type, exclusive, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order,supplier_status, goods_number, integral, " .
                    ...
                    " FROM " . $GLOBALS['ecs']->table('goods') . " AS g,".$GLOBALS['ecs']->table('virtual_district')." as dis, ".
                        $GLOBALS['ecs']->table('virtual_goods_district')." as dig".
                        " WHERE dis.supplier_id = ".$_SESSION['supplier_id']."  and dis.goods_id = g.goods_id and dis.district_id = dig.district_id  AND is_delete='$is_delete' AND g.supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
         }else{
             $sql = "SELECT distinct(goods_id), goods_name, goods_type, exclusive, goods_sn, shop_price, is_on_sale, is_best, is_new, is_hot, sort_order,supplier_status, goods_number, integral, " .
                    " (promote_price > 0 AND promote_start_date <= '$today' AND promote_end_date >= '$today') AS is_promote ".
                    " FROM " . $GLOBALS['ecs']->table('goods') . "  WHERE  is_delete='$is_delete' AND supplier_id='". $_SESSION['supplier_id'] ."' $where" .
                    " ORDER BY $filter[sort_by] $filter[sort_order] ".
                    " LIMIT " . $filter['start'] . ",$filter[page_size]";
         }
          /* 代码增加_虚拟团购_END  www.68ecshop.com */
