...
elseif($act == 'city_name_suggest')
{
    ...
    {
        //艺龙网的数据
        $remote_url = 'http://travel.elong.com/hotel/city/hotel/10.html?callback=jsonp'.intval(time()) . '&keyword='.$keyword;
        $result = file_get_contents($remote_url);
        $result = trim($result,'jsonp'.intval(1482986152830).'(');
        $result = trim($result,')');
...
elseif($act == 'hotsuggest')
{
    ...
    $city_cn = $_REQUEST['city_cn'] ? $_REQUEST['city_cn'] : '';
    if(!$city_cn)
    {
        $response['err'] = 1;
        $response['msg'] = '请输入城市中文名';
    }
    else
    {
        $city_cn = trim($city_cn);
        $file_name = 'data/hotel/city.json';
        if(!file_exists($file_name))
        {
            $response['err'] = 1;
            $response['msg'] = '城市推荐数据不存在,请联系管理员';
        }
        else
        {
            $citys_info = file_get_contents('data/hotel/city.json');
            $citys_info = json_decode($citys_info,true);
        }
    }
}
elseif($act == 'getCityId')
{
    //城市热门推荐,用户关键词的推荐框
    $response = array(
        'err' => 0,
        ...
        else
        {
            ...
            foreach($citys_info as $key => $val)
            {
                if($loop)
                {
                    ...
                    {
                        if($v['cityNameCn'] == trim($city_cn))
                        {
                            $response['msg']['cityid'] = $v['cityId'];
                            $response['msg']['citycn'] = $city_cn;
                            $loop = false;
                            break;
                        }
                    }
...
//
//if (!$smarty->is_cached('elong/index.html', $cache_id))
//{
    //assign_template();
    ...
    /* 页面中的动态内容 */
    assign_dynamic('elong/index.html');
//}
