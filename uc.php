...
function action_order_list ()
{
	...
	print_r($orders);
	$smarty->display('uc/my_order.html');
}
...
//		$smarty->assign('payment_list', $payment_list);
	...
	/* 增值税发票_添加_END_www.68ecshop.com */
	$smarty->assign('order', $order);
	$create_time = $order['formated_add_time'];
	$create_time = explode(' ', $create_time);
	$order['formated_add_time'] = $create_time[0] . '<br/>' . $create_time[1];
	print_r($order);
	/* 代码增加_start By www.68ecshop.com */
	foreach($goods_list as $goods_key => $goods_val)
	{
...
function action_account_log ()
{
	...
	$smarty->assign('surplus_yue', $surplus_yue);
	$smarty->assign('pager', $pager);
	$smarty->display('uc/my_account.html');
...
function action_coin_log ()
{
	...
	// $smarty->assign('surplus_yue', $surplus_yue);
	// $smarty->assign('pager', $pager);
	$smarty->display('uc/my_coin.html');
...
function action_my_comment()
{
	...
	if($o_id)
	{
		...
		$sql = "SELECT og.*, o.add_time, o.shipping_time_end, o.order_id, g.goods_thumb, s.shaidan_id, s.pay_points AS shaidan_points, s.status AS shaidan_status,c.comment_rank AS rank,c.content,
			c.status AS comment_status,c.comment_id,g.supplier_id,ifnull(ssc.value,'网站自营') AS shopname
	...
	while($row = $db->fetchRow($res))
	{
		...
		//$row['create_time'] = date('Y-m-d H:i:s',$row['add_time'])
		$item_list[] = $row;
	}
	// 代码增加 for 循环
	...
	$pager = get_pager('uc.php', array(
		'act' => $action
	), $count, $page, $size);
	$smarty->assign('min_time', $min_time);
	$smarty->assign('pager', $pager);
	$smarty->display('uc/my_jlcomt.html');
