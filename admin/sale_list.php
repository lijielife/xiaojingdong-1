...
if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'query' ||  $_REQUEST['act'] == 'download'))
{
    ...
    if ($_REQUEST['act'] == 'download')
    {
		...
		echo ecs_iconv(EC_CHARSET, 'GB2312', '商品货号') . "\t";
		echo ecs_iconv(EC_CHARSET, 'GB2312', '商品库存') . "\t";
		echo ecs_iconv(EC_CHARSET, 'GB2312', '折后价格') . "\t";
        echo ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['amount']) . "\t";
        echo ecs_iconv(EC_CHARSET, 'GB2312', $_LANG['sell_price']) . "\t";
		echo ecs_iconv(EC_CHARSET, 'GB2312', '进货成本') . "\t";
        ...
        foreach ($goods_sales_list['sale_list_data'] AS $key => $value)
        {
		   ...
		   echo ecs_iconv(EC_CHARSET, 'GB2312', $value['goods_name']) . "\t";
			...
			echo ecs_iconv(EC_CHARSET, 'GB2312', $value['shipping_fee']) . "\t";
...
***/		   
           ...
           'AS sales_price,og.cb_price, (og.goods_price-og.cb_price)*og.goods_number as gross_profit,oi.add_time AS sales_time,oi.order_id,oi.shipping_fee AS shipping_fee,oi.goods_amount AS goods_amount, oi.order_sn '.
           "FROM (SELECT og.goods_id, g.goods_number,og.goods_sn, og.goods_name, og.goods_number AS orders, g.cb_price, g.shop_price AS goods_price,og.order_id AS order_id,og.goods_price AS sales_price
...
while ($items = $GLOBALS['db']->fetchRow($sale_list_data))
{
		$sale_total += $items['sales_price'] * $items['goods_num'];
			$cost_total += $items['cb_price'] * $items['goods_num'];
			$gross_profit_total += $items['gross_profit'];
		$items['cb_price']   = price_format($items['cb_price']);
		...
		if($sale_total>0)
		{
		$gross_profit_rate = round($gross_profit_total*100/$sale_total,2).'%';   //成本毛利率
		$t = $sale_total - $cost_total;
		$sale_total   = price_format($sale_total);//销售收入
		$cost_total   = price_format($cost_total);//成本汇总
		$gross_profit_total   = price_format($t);//毛利润
