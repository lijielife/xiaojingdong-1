 ...
 * ==========================================================
 * $Author: liubo $
 * $Id: flashplay_p.php 17217 2011-01-19 06:29:08Z liubo $
 */
...
if ($_REQUEST['act']== 'list')
{
    ...
    {
        ecs_header("Location: flashplay_p.php?act=custom_list\n");
        exit;
    }
    ...
    /* 标签初始化 */
    $group_list = array(
        'sys' => array('text' => '默认', 'url' => ''),
        //'cus' => array('text' => $_LANG['custom_set'], 'url' => 'flashplay_p.php?act=custom_list')
                       );
    ...
    $smarty->assign('ur_here', $_LANG['flashplay_p']);
    $smarty->assign('action_link_special', array('text' => '添加图片', 'href' => 'flashplay_p.php?act=add'));
    ...
    $smarty->display('flashplay_p_list.htm');
}
elseif($_REQUEST['act']== 'del')
{
    ...
    {
        $links[] = array('text' => $_LANG['go_url'], 'href' => 'flashplay_p.php?act=list');
    ...
    ecs_header("Location: flashplay_p.php?act=list\n");
    exit;
}
elseif ($_REQUEST['act'] == 'add')
{
    ...
    if (empty($_POST['step']))
    {
        ...
        $smarty->assign('action_link', array('text' => '轮播图片列表', 'href' => 'flashplay_p.php?act=list'));
        $smarty->assign('rt', $rt);
        $smarty->assign('ur_here', $_LANG['add_picad']);
        $smarty->display('flashplay_p_add.htm');
    }
    elseif ($_POST['step'] == 2)
    {
        ...
        {
            $links[] = array('text' => $_LANG['add_new'], 'href' => 'flashplay_p.php?act=add');
...
elseif ($_REQUEST['act'] == 'edit')
{
    ...
    if (empty($_POST['step']))
    {
        ...
        $rt['id'] = $id;
        $smarty->assign('action_link', array('text' => $_LANG['go_url'], 'href' => 'flashplay_p.php?act=list'));
        $smarty->assign('rt', $rt);
    ...
    elseif ($_POST['step'] == 2)
    {
        ...
        {
            //若链接地址为空
            $links[] = array('text' => $_LANG['return_edit'], 'href' => 'flashplay_p.php?act=edit&id=' . $id);
            sys_msg($_LANG['link_empty'], 0, $links);
        }
...
elseif ($_REQUEST['act']== 'custom_list')
{
    /* 标签初始化 */
    $group_list = array(
        'sys' => array('text' => $_LANG['system_set'], 'url' => ($_CFG['index_ad'] == 'cus') ? 'javascript:system_set();void(0);' : 'flashplay_p.php?act=list'),
...
//        }
    ...
    $smarty->assign('action_link_special', array('text' => $_LANG['add_flash'], 'href' => 'flashplay_p.php?act=custom_add'));
...
//        }
    ...
    $smarty->assign('action_link_special', array('text' => $_LANG['add_flash'], 'href' => 'flashplay_p.php?act=custom_add'));
    $smarty->assign('action_link', array('text' => $_LANG['ad_play_url'], 'href' => 'flashplay_p.php?act=custom_list'));
    /* 添加 */
    $ad = array('ad_name' => '', 'ad_type' => 0, 'ad_url' => 'http://', 'htmls' => '',
                'ad_status' =>'1', 'ad_id' => '0', 'url' => 'http://');
    $smarty->assign('ad', $ad);
...
elseif ($_REQUEST['act'] == 'custom_insert')
{
    ...
    {
        $links[] = array('text' => $_LANG['back'], 'href' => 'flashplay_p.php?act=custom_list');
    ...
    $links[] = array('text' => $_LANG['back_custom_set'], 'href' => 'flashplay_p.php?act=custom_list');
    sys_msg($_LANG['edit_ok'], 0, $links);
}
...
elseif ($_REQUEST['act'] == 'custom_status')
{
    ...
    if (modfiy_ad_status($id, $ad_status))
    {
        ...
        $group_list = array(
            'sys' => array('text' => $_LANG['system_set'], 'url' => ($shop_config['value'] == 'cus') ? 'javascript:system_set();void(0);' : 'flashplay_p.php?act=list'),
            'cus' => array('text' => $_LANG['custom_set'], 'url' => '')
                           );
...
function put_flash_xml($flashdb)
{
    if (!empty($flashdb))
    {
        ...
        $xml .= '</bcaster>';
        file_put_contents(ROOT_PATH . DATA_DIR . '/flash_data_p.xml', $xml);
    }
    else
    {
        @unlink(ROOT_PATH . DATA_DIR . '/flash_data_p.xml');
    }
}
