...
function action_top ()
{
	...
	$filter = array();
	$where = " WHERE oi.user_id = u.user_id  AND oi.money_paid > 0 AND oi.order_status IN ('1','5') AND oi.pay_status IN ('2','1') GROUP BY oi.user_id" ;


	$sql = "SELECT DISTINCT(oi.user_id) AS num FROM " .
		$GLOBALS['ecs']->table('order_info') . ' AS oi,'.
		$GLOBALS['ecs']->table('users') . ' AS u '.
		$where ;
	$d = $GLOBALS['db']->query($sql);

	$filter['record_count'] = $GLOBALS['db']->num_rows($d);
	$filter = page_and_size($filter);


	$sql = "SELECT  SUM(oi.money_paid) AS money,u.user_name AS user_name ,u.froms FROM " .
		$GLOBALS['ecs']->table('order_info')." AS oi, ".
		$GLOBALS['ecs']->table('users') . ' AS u '.
		$where . " ORDER by money DESC  LIMIT " .  $filter['page_size'];
	...
	foreach ($data as $key => $item)
	{
		if($data[$key]['money'] > 0)
		{
			$sale_list_data[$key]['money'] = $data[$key]['money'];
			$sale_list_data[$key]['froms'] = $data[$key]['froms'];
			$sale_list_data[$key]['user_name'] = $data[$key]['user_name'];
		}
	}
	...
	$smarty->assign('user_list',    $sale_list_data);
	$smarty->assign('ur_here',      $_LANG['sell_stats']);
	$smarty->assign('full_page',        1);
	$smarty->assign('ur_here',      '会员消费统计排行榜');
	$smarty->assign('cfg_lang',     $_CFG['lang']);

	assign_query_info();
	$smarty->display('users_top.htm');
...
function action_top_query ()
{
	...
	$sql = "SELECT  SUM(oi.money_paid) AS money,u.user_name AS user_name ,u.froms FROM " .
		$GLOBALS['ecs']->table('order_info')." AS oi, ".
		$GLOBALS['ecs']->table('users') . ' AS u '.
		$where . " ORDER by money DESC  LIMIT " .  $filter['start'] . ',' . $filter['page_size'];
	...
	make_json_result($smarty->fetch('users_top.htm'), '', array(
		'filter' => $filter,'page_count' => $filter['page_count']
	));
}
