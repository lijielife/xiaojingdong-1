...
function action_register ()
{
	...
	else
	{
		...
		if((intval($_CFG['captcha']) & CAPTCHA_REGISTER) && gd_version() > 0)
		{
			if(empty($_POST['captcha']))
			{
				show_message($_LANG['invalid_captcha'], $_LANG['sign_up'], ''', 'error');
			}
		...
		**/
		if($register_type == "email")
		{
			...
			if(empty($email))
			{
				show_message($_LANG['msg_email_blank'], $_LANG['sign_up'], 'reg.php', 'error');
			}
			...
			if(empty($email_code))
			{
				show_message($_LANG['msg_email_code_blank'], $_LANG['sign_up'], 'reg.php', 'error');
			}
			else if($session_email != $email)
			{
				show_message($_LANG['email_changed'], $_LANG['sign_up'], 'reg.php', 'error');
			}
			else if($email_code != $record['record_code'])
			{
				show_message($_LANG['invalid_email_code'], $_LANG['sign_up'], 'reg.php', 'error');
			}
		...
		else if($register_type == "mobile")
		{
			...
			{
				show_message($_LANG['msg_mobile_phone_blank'], $_LANG['sign_up'], 'reg.php', 'error');
			}
			// 检查发送短信验证码的手机号码和提交的手机号码是否匹配
			else if($session_mobile_phone != $mobile_phone)
			{
				//show_message($_LANG['mobile_phone_changed'], $_LANG['sign_up'], 'reg.php', 'error');
			}
			// 检查验证码是否正确
			else if($record['record_code'] != $mobile_code)
			{
				show_message($_LANG['invalid_mobile_phone_code'], $_LANG['sign_up'], 'reg.php', 'error');
			}
			// 检查过期时间
		...
		{
			/* 无效的注册类型 */
			show_message($_LANG['register_type_invalid'], $_LANG['sign_up'], 'reg.php', 'error');
		}
		...
		{
			$GLOBALS['err']->show($_LANG['sign_up'], 'reg.php');
		}
	}
	/* 代码增加2014-12-23 by www.68ecshop.com _star */
}
