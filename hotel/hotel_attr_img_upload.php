...
{
    echo '<center><br>请先保存酒店,然后 1、选择编辑酒店,2、再上传图片</center>';
    exit;
}
...
if($_REQUEST['act']=="upload")
{
    ...
    if(!$goods_attr_id)
    {
        $array = handle_gallery_image_attr($goods_id, $goods_attr_id, $_FILES['file'],180,135);
    }
    else
    {
        $array = handle_gallery_image_attr($goods_id, $goods_attr_id, $_FILES['file'],80,80);
    }
...
elseif ($_REQUEST['act'] == 'drop_image')
{
    ...
    $sql = "SELECT img_url, thumb_url, img_original " .
        " FROM " . $GLOBALS['ecs']->table('hotels_gallery') .
        " WHERE img_id = '$img_id'";
    ...
    /* 删除数据 */
    $sql = "DELETE FROM " . $GLOBALS['ecs']->table('hotels_gallery') . " WHERE img_id = '$img_id' LIMIT 1";
...
elseif ($_REQUEST['act'] == 'set_attrimage')
{
    ...
    $sql_www_ecshop68_com = "update ". $GLOBALS['ecs']->table('hotels_gallery') ." set is_attr_image='0' where goods_id='$goods_id' and goods_attr_id='$goods_attr_id' ";
    $db->query($sql_www_ecshop68_com);
    $sql_www_ecshop68_com = "update ". $GLOBALS['ecs']->table('hotels_gallery') ." set is_attr_image='1' where img_id='$img_id_www_ecshop68_com' ";
    $db->query($sql_www_ecshop68_com);
    clear_cache_files();
}
...
?>
    ...
    <div class="gallery_box">
        ...
        $img_list =array();
        $sql = "SELECT * FROM " . $ecs->table('hotels_gallery') . " WHERE goods_attr_id= '$goods_attr_id' and goods_id = '$goods_id' order by img_sort,img_id ";
    ...
    <form name="setsort" id="setsort" action="hotel_attr_img_upload.php" method="post">
    ...
    <script type="text/javascript">
        ...
        $('#test').diyUpload({
            url:'hotel_attr_img_upload.php?goods_id=<?php echo $goods_id;?>&goods_attr_id=<?php echo $goods_attr_id;?>&act=upload',
 ...
 * @return  void
 */
function handle_gallery_image_attr($goods_id, $goods_attr_id, $image_files,$thumb_width='',$thumb_height='')
{
    /* 是否处理缩略图 */
    ...
    if ($flag)
    {
        ...
        if ($proc_thumb)
        {
            //$thumb_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'], ,  $GLOBALS['_CFG']['thumb_height']);
            if(!$thumb_width)
            {
                $thumb_width = $GLOBALS['_CFG']['thumb_width'];
            }
            if(!$thumb_height)
            {
                $thumb_height = $GLOBALS['_CFG']['thumb_height'];
            }
            $thumb_url = $GLOBALS['image']->make_thumb($image_files['tmp_name'], $thumb_width, $thumb_height);
        ...
        $sql = "INSERT INTO " . $GLOBALS['ecs']->table('hotels_gallery') . " (goods_id, img_url, img_desc, thumb_url, img_original, goods_attr_id) " .
        ...
        {
            $GLOBALS['db']->query("UPDATE " . $GLOBALS['ecs']->table('hotels_gallery') . " SET img_original='' WHERE `goods_id`='{$goods_id}'");
            @unlink('../' . $img_original);
        }
    }
