...
if ($_REQUEST['act'] == 'manage')
{
    ...
    $smarty->assign('action_link',      array('text' => '新建酒店类型', 'href' => 'hotels_type.php?act=add'));

    $smarty->display('hotels_type.htm');
}
...
elseif ($_REQUEST['act'] == 'add')
{
    ...
    $smarty->assign('action_link', array('href'=>'hotels_type.php?act=manage', 'text' => '酒店类型列表'));
    $smarty->assign('action',      'add');
    $smarty->assign('form_act',    'insert');
    $smarty->assign('goods_type',  array('enabled' => 1));

    assign_query_info();
    $smarty->display('hotels_type_info.htm');
}
...
elseif ($_REQUEST['act'] == 'update')
{
    ...
    if ($db->autoExecute($ecs->table('hotels_type'), $goods_type, 'UPDATE', "cat_id='$cat_id'") !== false)
    {
        /* 对比原来的分组 */
...
function get_goodstype()
{
    ...
    if ($result === false)
    {
        ...
        /* 记录总数以及页数 */
        $sql = "SELECT COUNT(*) FROM ".$GLOBALS['ecs']->table('hotels_type') . "WHERE type='service'";
...
{
    $sql = "SELECT * FROM " .$GLOBALS['ecs']->table('hotels_type'). " WHERE cat_id='$cat_id'";
...
{
    $sql = "UPDATE " . $GLOBALS['ecs']->table('hotels_attribute') .
