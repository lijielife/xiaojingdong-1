...
$exc = new exchange($ecs->table("rooms_attribute"), $db, 'attr_id', 'attr_name');
...
if ($_REQUEST['act'] == 'list')
{
    ...
    $smarty->assign('action_link',      array('href' => 'rooms_attribute.php?act=add&goods_type='.$goods_type , 'text' => $_LANG['10_attribute_add']));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('rooms_attribute_list.htm');
}
...
elseif ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'edit')
{
    ...
    {
        $sql = "SELECT * FROM " . $ecs->table('rooms_attribute') . " WHERE attr_id = '$_REQUEST[attr_id]'";
        $attr = $db->getRow($sql);
    }

    $smarty->assign('attr', $attr);
    $smarty->assign('attr_groups', get_rooms_attr_groups($attr['cat_id']));

    /* 取得商品分类列表 */
    $smarty->assign('goods_type_list', goods_rooms_type_list($attr['cat_id']));
    ...
    /* 显示模板 */
    assign_query_info();
    $smarty->display('rooms_attribute_info.htm');
}
...
elseif ($_REQUEST['act'] == 'insert' || $_REQUEST['act'] == 'update')
{
    ...
    if ($is_insert)
    {
        $db->autoExecute($ecs->table('rooms_attribute'), $attr, 'INSERT');
    ...
    {
        $db->autoExecute($ecs->table('rooms_attribute'), $attr, 'UPDATE', "attr_id = '$_POST[attr_id]'");
	...
	{
		$sql_www_ecshop68_com="update " .$ecs->table("rooms_attribute"). " set  is_attr_gallery=0 where cat_id='".$_POST['cat_id']."' ";
		$db->query($sql_www_ecshop68_com);
	}
	$sql_www_ecshop68_com="update " .$ecs->table("rooms_attribute"). " set  is_attr_gallery='$_POST[is_attr_gallery]' where attr_id='$attr_id_www_ecshop68_com' ";
	$db->query($sql_www_ecshop68_com);
...
elseif ($_REQUEST['act'] == 'batch')
{
    ...
    if (isset($_POST['checkboxes']))
    {
        ...
        $sql = "DELETE FROM " . $ecs->table('rooms_attribute') . " WHERE attr_id " . db_create_in($ids);
        $db->query($sql);

        $sql = "DELETE FROM " . $ecs->table('rooms_goods_attr') . " WHERE attr_id " . db_create_in($ids);
        $db->query($sql);
        ...
        $link[] = array('text' => $_LANG['back_list'], 'href' => 'rooms_attribute.php?act=list');
...
elseif ($_REQUEST['act'] == 'remove')
{
    ...
    $db->query("DELETE FROM " .$ecs->table('rooms_attribute'). " WHERE attr_id='$id'");
    $db->query("DELETE FROM " .$ecs->table('rooms_attr'). " WHERE attr_id='$id'");

    $url = 'rooms_attribute.php?act=query&' . str_replace('act=remove', '', $_SERVER['QUERY_STRING']);
...
elseif ($_REQUEST['act'] == 'get_attr_num')
{
    ...
    $sql = "SELECT COUNT(*) ".
           " FROM " . $ecs->table('rooms_attr') . " AS a, ".
           $ecs->table('rooms') . " AS g ".
...
{
	$sql = "delete from ". $ecs->table('rooms_attribute_color') ." where attr_id= '$_REQUEST[attr_id]' ";
	$db->query($sql);
	foreach ($_REQUEST['color_name'] AS $color_key=> $color_name)
	{
		if ($_REQUEST['color_code'][$color_key])
		{
			$sql="insert into ". $ecs->table('rooms_attribute_color') ."(attr_id, color_name, color_code) values('$_REQUEST[attr_id]', '$color_name', '". $_REQUEST['color_code'][$color_key] ."' )";
			$db->query($sql);
		}
	}
	$link[] = array('text' => '返回设置页面', 'href' => 'rooms_attribute.php?act=setcolor&attr_id='.$_REQUEST['attr_id']);
...
elseif ($_REQUEST['act'] == 'setcolor')
{
	...
	$colors_code=array();
	$sql="select * from ". $ecs->table('rooms_attribute_color') . " where attr_id = '$_REQUEST[attr_id]'";
    ...
    $smarty->assign('action_link', array('href' => 'rooms_attribute.php?act=list&goods_type='.$attr['cat_id'], 'text' => $_LANG['09_attribute_list']));
	 /* 显示模板 */
    assign_query_info();
...
function get_attrlist()
{
    ...
    $sql = "SELECT COUNT(*) FROM " . $GLOBALS['ecs']->table('rooms_attribute') . " AS a $where";
    ...
    $sql = "SELECT a.*, t.cat_name " .
            " FROM " . $GLOBALS['ecs']->table('rooms_attribute') . " AS a ".
            " LEFT JOIN " . $GLOBALS['ecs']->table('rooms_type') . " AS t ON a.cat_id = t.cat_id " . $where .
 ...
 * @return  array
 */
function get_rooms_attr_groups($cat_id)
{
    $sql = "SELECT attr_group FROM " . $GLOBALS['ecs']->table('rooms_type') . " WHERE cat_id='$cat_id'";
 ...
 * @return  string
 */
function goods_rooms_type_list($selected)
{
    $sql = 'SELECT cat_id, cat_name FROM ' . $GLOBALS['ecs']->table('rooms_type') . ' WHERE enabled = 1';
