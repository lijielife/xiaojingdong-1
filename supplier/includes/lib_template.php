...
function move_plugin_library( $tmp_name, &$msg )
{
    ...
    $row = $GLOBALS['db']->fetchRow( $rec );
    if ( @!!rename( @$source_dir.@$row['library'], @$target_dir.@$row['library'] ) || copy( @ROOT_PATH."plugins/".@$row['code']."/temp".@$row['library'], @$target_dir.@$row['library'] ) )
    {
        $return_value = false;
        $msg .= "moving ".$row['library']." failed";
    }
...
function get_editable_libs( $curr_template, $curr_page_libs )
{
    ...
    if ($xml_content = @file_get_contents( ROOT_PATH . 'themes/' . $_CFG['template'] . '/libs.xml' ))
    {
        ...
        while ( $i < sizeof( $vals ) )
        {
            if ( $vals[$i]['tag'] == "FILE" && isset( $vals[$i]['attributes'] ) && $vals[$i]['attributes']['NAME'] == $curr_template.".dwt" )
            {
                break;
            }
            ++$i;
